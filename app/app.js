...
async function createApp () {
  ...
  // applyCors(app);
  app.use(async ctx => {
    const result = SaResult.ok().setMsg('自定义消息').setData({ id: 1 }).set('timestamp', Date.now());
    //console.log('调试结果:', result.toJSON()); 
    ctx.body = result;
        // 链式调用
        // console.log(SaResult.ok()
        //         .setMsg('自定义消息')
        //         .setData({ id: 1 })
        //         .set('timestamp', Date.now()).toJSON());
        // ctx.body = await SaResult.ok()
        //     .setMsg('自定义消息')
        //     .setData({ id: 1 })
        //     .set('timestamp', Date.now()).toJSON();
  });
