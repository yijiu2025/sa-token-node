...
class BCrypt {
    ...
    // BCrypt 加密的初始向量（与 Java 一致）
    static bf_crypt_ciphertext = [
    0x4f727068, 0x65616e42, 0x65686f6c, 
    0x64657253, 0x63727944, 0x6f756274
    ];

    // Base64 编码表（与 Java 完全一致）
    static base64_code = './ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'.split('');

    // Base64 解码表
    static index_64 = (() => {
    const table = Array(128).fill(-1);
    base64_code.forEach((c, i) => table[c.charCodeAt(0)] = i);
    return table;
    })();


    /* ----------- 静态方法（与 Java 版完全一致的公开API）----------- */

    // static hashpw(password) {
    //     return this.hashpw(password, this.gensalt());
    // }

    static hashpw(password, salt = this.gensalt()) {
        let bcrypt;
        let real_salt;
        let saltb;
        let hashed;
        ...
        rounds = parseInt(salt.substring(off, off + 2), 10);

        real_salt = salt.substring(off + 3, off + 25);
        const passwordb = new TextEncoder().encode(
            password + (minor >= 'a' ? "\u0000" : "")
        );
        saltb = this.decodeBase64(real_salt, BCrypt.BCRYPT_SALT_LEN);

        bcrypt = new BCrypt();
        const ciphertext = [...this.bf_crypt_ciphertext];
        hashed = bcrypt.crypt(passwordb, saltb, rounds, ciphertext);
        ...
        rs.push('$');
        rs.push(this.encodeBase64(saltb, saltb.length));
        rs.push(this.encodeBase64(hashed, this.bf_crypt_ciphertext.length * 4 - 1));
        return rs.join('');
    }
     ...
     * 生成随机 salt
     *
     * @param {number} log_rounds - 日志轮数，即 2^log_rounds 次 Blowfish 加密轮数
     * @returns {string} - 格式为 "$2a$log_rounds$salt" 的 salt 字符串
     */
    // static gensalt(log_rounds) {
    //     return this.gensalt(log_rounds, crypto.getRandomValues(new Uint8Array(BCrypt.BCRYPT_SALT_LEN)));
    // }

    static gensalt(log_rounds = BCrypt.GENSALT_DEFAULT_LOG2_ROUNDS, random = crypto.getRandomValues) {
        const rs = [];
        const rnd = new Uint8Array(BCrypt.BCRYPT_SALT_LEN);
    
        // Fill with secure random bytes
        crypto.getRandomValues(rnd); // 填充密码学安全随机字节
        ...
        rs.push('$');
        rs.push(this.encodeBase64(rnd, rnd.length));
     ...
     * @param {string} hashed    - 已哈希的字符串（如 "$2a$10$..."）
    ...
    static checkpw(plaintext, hashed) {
        ...
        let try_pw;
        try {
            try_pw = this.hashpw(plaintext, hashed); // 假设 hashpw 是类的静态方法
        } catch (ignore) {
            // 生成密文时出错，直接返回 false
            return false;
        }

        // 模拟 Java 的 getBytes(StandardCharsets.UTF_8)
    ...
    static encodeBase64(d, len) {
        ...
        if (off >= len) {
            rs.push(base64_code[c1b & 0x3f]);
            break;
        }
        
        const c2 = d[off++] & 0xff;
        rs.push(base64_code[(c1b | (c2 >> 4)) & 0x3f]);
        const c2b = (c2 & 0x0f) << 2;
        
        if (off >= len) {
            rs.push(base64_code[c2b & 0x3f]);
            break;
        }
        
        const c3 = d[off++] & 0xff;
        rs.push(base64_code[(c2b | (c3 >> 6)) & 0x3f]);
        rs.push(base64_code[c3 & 0x3f]);
        }
        
        return rs.join('');
    }

    static decodeBase64(s, maxolen) {
        let off = 0;
        const result = [];
        
        while (off < s.length - 1 && result.length < maxolen) {
        const c1 = index_64[s.charCodeAt(off++)];
        const c2 = index_64[s.charCodeAt(off++)];
        if (c1 === -1 || c2 === -1) break;

        let o = (c1 << 2) >>> 0;
        o |= (c2 & 0x30) >> 4;
        result.push(o);
        if (result.length >= maxolen || off >= s.length) break;

        const c3 = index_64[s.charCodeAt(off++)];
        if (c3 === -1) break;
        ...
        const c4 = index_64[s.charCodeAt(off++)];
        o = ((c3 & 0x03) << 6) >>> 0;
        o |= c4;
        result.push(o);
        }
     ...
     * 加密密文（bcrypt 核心）
     *
     * @param {Uint8Array|number[]} password - 明文密码（字节数组）
     * @param {Uint8Array|number[]} salt     - 加盐（长度必须为 BCRYPT_SALT_LEN）
     * @param {number} log_rounds            - 迭代轮数的对数（通常 4~30）
     * @param {number[]} cdata               - 要加密的数据（32位整数数组）
     * @returns {Uint8Array}                 - 加密后的字节数组（大端序）
     */
    crypt(password, salt, log_rounds, cdata) {
        let rounds, i, j;
        ...
        // 主循环：多次使用 password 和 salt 更新 P/S 数组
        for (i = 0; i < rounds; i++) {
            this.key(password);
            this.key(salt);
        }

        // 对 cdata 进行 64 轮 Feistel 加密
        // 每次处理 clen/2 个 64 位块（即每两个 32 位整数组成一个块）
        for (i = 0; i < 64; i++) {
            for (j = 0; j < (clen >> 1); j++) {
                this.encipher(cdata, j << 1); // j*2 偏移
            }
        }

        // 将 32 位整数数组转换为字节数组（大端序）
        ret = new Uint8Array(clen * 4);
        for (i = 0, j = 0; i < clen; i++) {
            const val = cdata[i] >>> 0; // 确保无符号 32 位
            ret[j++] = (val >> 24) & 0xff;
            ret[j++] = (val >> 16) & 0xff;
            ret[j++] = (val >> 8) & 0xff;
            ret[j++] = val & 0xff;
    ...
    encipher(lr, off) {
        ...
        l ^= this.P[0];
        for (let i = 0; i <= BCrypt.BLOWFISH_NUM_ROUNDS - 2;) {
            // Feistel 网络（与 Java 相同实现）
            let n = this.S[(l >> 24) & 0xff];
    ...
    /**
     * 用指定密钥初始化 Blowfish 算法的 P 数组和 S 数组
     *
     * @param {Uint8Array|number[]} key - 密钥字节数组
     */
    key(key) {
        // 假设：this.P, this.S, this.streamToWord, this.encipher 已定义
        const plen = this.P.length;
        const slen = this.S.length;
        const koffp = [0]; // 模拟指针：存储当前密钥偏移
        const lr = [0, 0]; // 64位数据块：左右各32位
        ...
        // Step 2: 使用加密函数生成新的 P 数组
        for (let i = 0; i < plen; i += 2) {
            this.encipher(lr, 0);        // 加密当前 lr (初始为 0)
            this.P[i] = lr[0];           // 替换 P[i]
            this.P[i + 1] = lr[1];       // 替换 P[i+1]
        }

        // Step 3: 使用加密函数生成 S-box (共1024项，每次填2个)
        for (let i = 0; i < slen; i += 2) {
            this.encipher(lr, 0);        // 加密当前 lr
            this.S[i] = lr[0];           // 填入 S[i]
            this.S[i + 1] = lr[1];       // 填入 S[i+1]
        }
    }

  /**
     * 执行 bcrypt 的“增强密钥调度”（Ekskey）
     * 结合 salt 和 password 初始化 P 和 S 数组
     *
     * @param {Uint8Array|number[]} data - salt 数据（16字节）
     * @param {Uint8Array|number[]} key  - password 数据（字节数组）
     */
    ekskey(data, key) {
        const plen = this.P.length;  // 通常为 18
        const slen = this.S.length;  // 通常为 1024
        const koffp = [0];           // 密钥偏移指针
        const doffp = [0];           // salt 偏移指针
        const lr = [0, 0];           // 64位数据块（两个32位整数）

        // Step 1: 用 password 初始化 P 数组（异或）
        for (let i = 0; i < plen; i++) {
            this.P[i] = this.P[i] ^ this.streamToWord(key, koffp);
        }

        // Step 2: 使用 salt 混入并加密，填充 P 数组
        for (let i = 0; i < plen; i += 2) {
            lr[0] ^= this.streamToWord(data, doffp);  // 左半部分异或 salt 字
            lr[1] ^= this.streamToWord(data, doffp);  // 右半部分异或 salt 字
            this.encipher(lr, 0);                     // 加密 lr
            this.P[i] = lr[0];                        // 存回 P[i]
            this.P[i + 1] = lr[1];                    // 存回 P[i+1]
        }

        // Step 3: 使用 salt 混入并加密，填充 S-box
        for (let i = 0; i < slen; i += 2) {
            lr[0] ^= this.streamToWord(data, doffp);  // 继续从 salt 读取
            lr[1] ^= this.streamToWord(data, doffp);
            this.encipher(lr, 0);                     // 加密
            this.S[i] = lr[0];                        // 填入 S[i]
            this.S[i + 1] = lr[1];                    // 填入 S[i+1]
        }
    }
    ...
    /**
     * 循环地从字节数组中提取一个 32 位字（4 字节，大端序）
     *
     * @param {Uint8Array|number[]} data - 字节数组
     * @param {number[]} offp - 偏移量“指针”（单元素数组，如 [0]）
     * @returns {number} 一个 32 位整数（无符号）
     */
    streamToWord(data, offp) {
        let word = 0;
        let off = offp[0];  // 模拟指针解引用
    
        for (let i = 0; i < 4; i++) {
            // 左移 8 位，高位在前（Big-Endian），并取字节无符号值
            word = (word << 8) | (data[off] & 0xff);
            // 循环递增偏移：到达末尾时回到开头
            off = (off + 1) % data.length;
        }
    
        offp[0] = off;  // 更新偏移（模拟指针赋值）
        return word >>> 0;  // 确保返回无符号 32 位整数
    }
}
