...
class SaRawSessionUtil {
	...
	static splicingSessionKey(type, valueId) {
        return `${SaManager.default.getConfig().getTokenName()}:raw-session:${type}:${valueId}`;
    }
	...
	static isExists(type, valueId) {
        return SaManager.default.getSaTokenDao().getSession(this.splicingSessionKey(type, valueId)) != null;
    }
    ...
    static getSessionById(type, valueId, isCreate = true) {
        ...
        let session = SaManager.default.getSaTokenDao().getSession(sessionId);
        
        if (session == null && isCreate) {
            session = SaStrategy.instance.createSession(sessionId);
            session.setType(type);
            SaManager.default.getSaTokenDao().setSession(
                session, 
                SaManager.default.getConfig().getTimeout()
            );
        }
        return session;
    }
	...
	static deleteSessionById(type, valueId) {
        SaManager.default.getSaTokenDao().deleteSession(this.splicingSessionKey(type, valueId));
    }

}

module.exports = SaRawSessionUtil;
