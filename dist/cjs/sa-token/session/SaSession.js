...
var SaSetValueInterface = require('../application/SaSetValueInterface.js');
...
class SaSession extends SaSetValueInterface {
	...
	constructor(id) {
		...
		this.createTime = Date.now();
 		// $$ 发布事件
		SaTokenEventCenter.default.doCreateSession(id);
	}
	 ...
	 */
	async update() {
		const SaTokenDao = await SaManager.default.getSaTokenDao();
		SaTokenDao.updateSession(this);
	}

	/** 注销Session (从持久库删除) */
	logout() {
		SaManager.default.getSaTokenDao().deleteSession(this.id);
 		// $$ 发布事件 
		SaTokenEventCenter.default.doLogoutSession(id);
	}
	...
	updateTimeout(timeout) {
        SaManager.default.getSaTokenDao().updateSessionTimeout(this.id, timeout);
    }
