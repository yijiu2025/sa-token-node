...
var SaRawSessionDelegator = require('../session/raw/SaRawSessionDelegator.js');
var SaStrategy = require('../strategy/SaStrategy.js');
var SaTtlMethods = require('../util/SaTtlMethods.js');
...
class SaTempTemplate extends SaTtlMethods {
	...
	saveToken(token, value, timeout) {
		...
		SaManager.default.getSaTokenDao().setObject(key, value, timeout);
	}
	...
	createTempTokenValue(value) {
		return SaStrategy.instance.generateUniqueToken(
				"Temp Token",
				SaManager.default.getConfig().getMaxTryTimes(),
	...
	_getValue(token) {
		const key = this.splicingTempTokenSaveKey(token);
		return SaManager.default.getSaTokenDao().getObject(key);
	}
	
	_deleteToken(token) {
		const key = this.splicingTempTokenSaveKey(token);
		SaManager.default.getSaTokenDao().deleteObject(key);
	}
	
	_getTimeout(token) {
		const key = this.splicingTempTokenSaveKey(token);
		return SaManager.default.getSaTokenDao().getObjectTimeout(key);
	}
	...
	splicingTempTokenSaveKey(token) {
		return SaManager.default.getConfig().getTokenName() + ":" + this.namespace + ":" + token;
	}
