...
var SaFoxUtil = require('../util/SaFoxUtil.js');
var SaReplacedRange = require('./parameter/enums/SaReplacedRange.js');
var SaLogoutMode = require('./parameter/enums/SaLogoutMode.js');
var SaLogoutRange = require('./parameter/enums/SaLogoutRange.js');
var SaHolder = require('../context/SaHolder.js');
var SaCookie = require('../context/model/SaCookie.js');
var SaErrorCode = require('../error/SaErrorCode.js');
var SaTokenException = require('../exception/SaTokenException.js');
var SaResponse = require('../context/model/SaResponse.js');
var SaTokenDao = require('../dao/SaTokenDao.js');
var SaTokenEventCenter = require('../listener/SaTokenEventCenter.js');
var SaTokenConsts = require('../util/SaTokenConsts.js');
var SaManager = require('../SaManager.js');
var SaTerminalInfo = require('../session/SaTerminalInfo.js');
var SaLoginParameter = require('./parameter/SaLoginParameter.js');
var SaLogoutParameter = require('./parameter/SaLogoutParameter.js');
var SaStrategy = require('../strategy/SaStrategy.js');
var SaValue2Box = require('../util/SaValue2Box.js');
var SaTokenInfo = require('./SaTokenInfo.js');
var NotLoginException = require('../exception/NotLoginException.js');
var ApiDisabledException = require('../exception/ApiDisabledException.js');
var NotRoleException = require('../exception/NotRoleException.js');
var NotPermissionException = require('../exception/NotPermissionException.js');
var DisableServiceException = require('../exception/DisableServiceException.js');
var NotSafeException = require('../exception/NotSafeException.js');
...
class StpLogic {
	...
	setLoginType(loginType){
		...
		if(SaFoxUtil.isNotEmpty(this.loginType)) {
			SaManager.default.removeStpLogic(this.loginType);
		}
		...
		SaManager.default.putStpLogic(this);
	...
	async getConfigOrGlobal() {
		...
		const globalCfg = await SaManager.default.getConfig();
		return globalCfg;
	}
	...
	async createLoginSession(id, loginParameter = this.createSaLoginParameter()) {
		...
		SaTokenEventCenter.default.doLogin(this.loginType, id, tokenValue, loginParameter);
	...
	async checkLoginArgs(id, loginParameter) {
		...
		if( ! SaFoxUtil.isBasicType(typeof id)) {
			SaManager.default.log.warn("loginId 应该为简单类型，例如：String | int | long，不推荐使用复杂类型：" + typeof id);
		}
		...
		if( ! this.isSupportExtra()) {
			...
			if(loginParameter.haveExtraData()) {
				SaManager.default.log.warn("当前 StpLogic 不支持 extra 扩展参数模式，传入的 extra 参数将被忽略");
			}
		}
		...
		if( ! cfg.getDynamicActiveTimeout() && loginParameter.getActiveTimeout() != null) {
			SaManager.default.log.warn("当前全局配置未开启动态 activeTimeout 功能，传入的 activeTimeout 参数将被忽略");
		}
	...
	_logoutByTokenValue(tokenValue, logoutParameter) {
		...
		if(logoutParameter.getMode() == SaLogoutMode.LOGOUT) {
			...
			SaTokenEventCenter.default.doLogout(this.loginType, loginId, tokenValue);
		}
		...
		if(logoutParameter.getMode() == SaLogoutMode.KICKOUT) {
			...
			SaTokenEventCenter.default.doKickout(this.loginType, loginId, tokenValue);
		}
		...
		if(logoutParameter.getMode() == SaLogoutMode.REPLACED) {
			...
			SaTokenEventCenter.default.doReplaced(this.loginType, loginId, tokenValue);
		}
	...
	_logout(loginId, logoutParameter) {
		...
		if(session != null) {
			...
			if(logoutParameter.getMode() == SaLogoutMode.REPLACED) ; else {
				session.logoutByTerminalCountToZero();
			}
		}
	}
	...
	_renewTimeout(tokenValue, timeout) {
		...
		SaTokenEventCenter.default.doRenewTimeout(this.loginType, loginId, tokenValue, timeout);
	}
	...
	getRoleList(loginId = this.getLoginId()) {
		return SaManager.default.getStpInterface().getRoleList(loginId, this.loginType);
	}
	...
	getPermissionList(loginId = this.getLoginId()) {
		return SaManager.default.getStpInterface().getPermissionList(loginId, this.loginType);
	}
	...
	untieDisable(loginId, ...services) {
		...
		for (const service of services) {
			...
			// $$ 发布事件
			SaTokenEventCenter.default.doUntieDisable(this.loginType, loginId, service);
		}
	}
	...
	disableLevel(loginId, serviceOrLevel, levelOrTime, time) {
		...
		// 发布事件
		SaTokenEventCenter.default.doDisable(
			this.loginType,
			loginId, 
			serviceOrLevel,
			levelOrTime,
	...
	getDisableLevel(loginId, service = SaTokenConsts.DEFAULT_DISABLE_SERVICE) {
		...
		const disableWrapperInfo = SaManager.default.getStpInterface().isDisabled(loginId, service);
	...
	openSafe(serviceOrSafeTime, safeTime) {
		...
		// 5、发布事件
		SaTokenEventCenter.default.doOpenSafe(
			this.loginType,
			tokenValue,
			service,
			timeout
	...
	closeSafe(service = SaTokenConsts.DEFAULT_SAFE_AUTH_SERVICE) {
		...
		SaTokenEventCenter.default.doCloseSafe(this.loginType, tokenValue, service);
	}
	 ...
	 */
	getSaTokenDao() {
		return SaManager.default.getSaTokenDao();
	}
