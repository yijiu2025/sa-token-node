...
var NotImplException = require('../exception/NotImplException.js');
var SaTokenException = require('../exception/SaTokenException.js');
var SaSession = require('../session/SaSession.js');
var StpLogic = require('../stp/StpLogic.js');
...
class SaStrategy {
	...
	createToken = async (loginId, loginType) => {
		...
		const tokenStyle = (await SaManager.default.getStpLogic(loginType).getConfigOrGlobal()).getTokenStyle();
		...
		switch (tokenStyle) {
			...
			// 默认，还是uuid
			default:
				SaManager.default.getLog().warn("配置的 tokenStyle 值无效：{}，仅允许以下取值: " +
